...
class Agent:
    ...
    def policyEvaluation(self):
        while True:
            ...
            for s in self.world.all_states():
                ...
                a = self.policy[s]  # deterministic policy
                new_v = 0.0

                # sum over stochastic transitions
                for s_next, p_env in self.world.probs[(s, a)].items():
                    r = self.world.rewards.get(s_next, 0)
                    new_v += p_env * (r + self.gamma * self.Vs[s_next])
    ...
    def policyImprovement(self):
        ...
        for s in self.world.all_states():
            ...
            # greedy update
            best_a = max(self.Qsa[s], key=self.Qsa[s].get)
    ...
    def policyIteration(self):
        while True:
            ...
            is_stable = self.policyImprovement()
            if is_stable:
                break
...
# -----------------------------
def main():
    world = grid_world.windy_grid_penalized(-0.1)
