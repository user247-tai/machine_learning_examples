...
def get_return_from_env_worker(args):
    ...
    while not done:
        ...
        # Policy forward pass
        action = np.argmax(net.forward(normalized_obs), axis=0)
...
class ARSAgent():
    ...
    def __init__(self, env_name="CartPole-v1"):
        ...
        self.n_directions = 30   # Number of noise vectors (N)
        self.b = 15              # Number of top performing directions to keep (Elites)
        ...
        self.act_size = temp_env.action_space.n
        temp_env.close()
    ...
    def test(self, episodes=1, render=True):
        ...
        for e in range(episodes):
            ...
            while not done:
                ...
                action = np.argmax(self.net.forward(normalized_obs), axis=0)
...
def main():
    ...
    agent.save_model("cartpole.npz")

    # Load and test
    try:
        agent.load_model("cartpole.npz")
