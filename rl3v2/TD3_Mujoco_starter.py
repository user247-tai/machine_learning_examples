...
class TD3Agent:
    def __init__(self):
        ...
        self.target_action_noise_min = -0.5
        self.target_action_noise_max = 0.5
        ...
        self.critic_optimizer = optim.AdamW(list(self.critic_net1.parameters()) + list(self.critic_net2.parameters()), lr=self.learning_rate)
    ...
    def make_action_tensor_noisy(self, action_tensor):
        noise = torch.randn_like(action_tensor) * self.target_action_noise
        noise = noise.clamp(self.target_action_noise_min, self.target_action_noise_max)
        action_tensor = action_tensor + noise
        action_tensor = torch.max(torch.min(action_tensor, self.action_high), self.action_low)
    ...
    def optimize(self):
        ...
        with torch.no_grad():
            ...
            next_actions = self.make_action_tensor_noisy(next_actions)

            target_critic_input = torch.cat((next_state_batch, next_actions), dim=1)
            next_q1 = self.target_critic_net1(target_critic_input).squeeze(-1)
            next_q2 = self.target_critic_net2(target_critic_input).squeeze(-1)
        ...
        current_q1 = self.critic_net1(critic_input).squeeze(-1)
        current_q2 = self.critic_net2(critic_input).squeeze(-1)
        ...
        if self.total_it % self.policy_frequency == 0:
            ...
            actor_loss = -self.critic_net1(torch.cat((state_batch, actions_pred), dim=1)).mean()
    ...
    # ===== SAVE / LOAD =====
    def save_model(self, path="cat.pth"):
        torch.save(self.actor_net.state_dict(), "actor_" + path)

    def load_model(self, path="cat.pth"):
